...
import { generateResearchOutline, streamSectionDraft, rewriteSection, generateAgentResponse } from '../services/geminiService';
...
export const SidebarRight: React.FC<SidebarRightProps> = ({ 
  ...
  const handleSendMessage = async (textOverride?: string) => {
    ...
    if (setAgentState) setAgentState(AgentState.THINKING);

    try {
        const paperContext = projectPapers.map(p => `Paper: ${p.title}\nAbstract: ${p.summary}`).join('\n\n');
        let toolContext = "";

        if (activeTab === 'PLAN') {
            toolContext = `CURRENT PLAN STATUS:\n${JSON.stringify(outline.map(s => ({ title: s.title, status: s.status })), null, 2)}`;
        } else if (activeTab === 'LIBRARY') {
            toolContext = `ACTIVE LIBRARY CONTEXT:\n${projectPapers.map(p => `"${p.title}" (${p.year}) - Authors: ${p.authors.join(', ')}`).join('\n')}`;
        } else if (activeTab === 'ASSETS') {
            toolContext = `AVAILABLE ASSETS:\n${activeProject?.assets.map(a => `${a.name} (${a.type})`).join(', ')}`;
        }

        const editorContext = activeFileContent ? `Current Draft (Last 3000 chars):\n${activeFileContent.slice(-3000)}` : 'Draft is empty.';
        const fullContext = `${paperContext}\n\n${toolContext}\n\n${editorContext}`;

        const response = await generateAgentResponse(text, fullContext);
        
        if (setAgentState) setAgentState(AgentState.SPEAKING);
        setTimeout(() => {
             setChatMessages(prev => [...prev, { role: 'agent', text: response }]);
             if (setAgentState) setAgentState(AgentState.IDLE);
        }, 800);
  ...
  const handlePdfChatSubmit = async () => {
    ...
    try {
        const paper = MOCK_PAPERS.find(p => p.id === activePaper);
        const context = paper ? `Paper Title: ${paper.title}\nAuthors: ${paper.authors.join(', ')}\nYear: ${paper.year}\nSummary: ${paper.summary}` : 'No paper context.';
        
        const response = await generateAgentResponse(text, context);
        
        if (setAgentState) setAgentState(AgentState.SPEAKING);
        setTimeout(() => {
            setPdfChatMessages(prev => [...prev, { role: 'agent', text: response }]);
            if (setAgentState) setAgentState(AgentState.IDLE);
        }, 600);
    } catch (e) {
        console.error(e);
  ...
  const handleGenerateOutline = async () => {
      ...
      try {
          const generatedOutline = await generateResearchOutline(projectPapers, activeProject.assets);
          setOutline(generatedOutline);
          if (addAgentLog) addAgentLog('Co-Author', `Generated ${generatedOutline.length} sections.`, 'success');
      } catch (e) { console.error(e); } 
      finally { setIsGeneratingOutline(false); }
  };

  // The Core Drafting Function
  const executeDraftSection = async (section: OutlineSection) => {
      if (draftingSectionId || !onUpdateSection) return;
      setDraftingSectionId(section.id);
      setAssetPromptSectionId(null);
      if (addAgentLog) addAgentLog('Co-Author', `Drafting: ${section.title}...`);
      
      const relevantPapers = MOCK_PAPERS.filter(p => section.relevantPaperIds.includes(p.id));
      const selectedAssets = activeProject?.assets.filter(a => draftingAssetIds.has(a.id)).map(a => a.name) || [];
      
      const stream = streamSectionDraft(section, relevantPapers, selectedAssets);
      let accumulatedText = "";
  ...
  const handleRewrite = async (section: OutlineSection, instruction: string) => {
      if (isRewriting || !instruction.trim()) return;
      setIsRewriting(true);
      ...
      const stream = rewriteSection(content, instruction);
      try {
          let accumulated = "";
          for await (const chunk of stream) {
              accumulated += chunk;
              setRewriteSuggestion(accumulated);
          }
      } finally {
          setIsRewriting(false);
      }
                                                            ...
                                                            <>
                                                                {assetPromptSectionId === section.id ? (
                                                                    ...
                                                                    <button 
                                                                        ...
                                                                        className={`w-full py-1.5 rounded text-[10px] font-bold uppercase tracking-wide flex items-center justify-center gap-2 transition-colors ${
                                                                            isDraftingThis ? 'bg-indigo-900/20 text-indigo-400' : 'bg-[#1e2025] text-gray-400 hover:bg-gray-800 hover:text-white border border-gray-800'
                                                                        }`}
                                                                    >
                        ...
                        logs.map((log) => (
                            <div key={log.id} className="animate-in fade-in slide-in-from-left-2 duration-300 group">
                                ...
                                <div className={`pl-2 border-l-2 ${
                                    log.source === 'Thought' ? 'border-amber-500 text-amber-500 italic' :
