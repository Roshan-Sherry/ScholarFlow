...
export const SidebarLeft: React.FC<SidebarLeftProps> = ({ 
  ...
  // Derived Assets
  const activePaperSummary = hoveredPaper ? projectPapers.find(p => p.id === hoveredPaper.id)?.summary : '';
      ...
      <aside className={`h-full flex flex-col ${borderClass} transition-colors duration-500 z-20 ${isStudio ? 'bg-black border-gray-800 text-gray-300' : 'bg-gray-50 border-gray-200 text-gray-700'}`}>
             ...
             <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                ...
                <div className="p-4 space-y-3 shrink-0 border-t border-gray-800/50">
                    <button 
                        ...
                        className={`w-full flex items-center justify-center gap-2 px-3 py-3 text-xs font-bold uppercase tracking-wide rounded-lg transition-all border ${
                            isAutoPilot 
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex-1 cursor-pointer min-w-0" onClick={() => onOpenPaper(paper.id)}>
                                        ...
                                        <div className="text-xs text-gray-500 mt-1 truncate">{paper.authors[0]} • {paper.year}</div>
                                    </div>
        ...
        <div className="p-4 border-t border-inherit bg-inherit shrink-0">
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 py-3 px-4 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${
              isStudio 
